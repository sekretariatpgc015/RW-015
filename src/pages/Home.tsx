import { ArrowRight, Info, Shield, Users, Calendar, MapPin, ChevronLeft, ChevronRight, MessageSquare, Send, User, MessageCircle, LogIn, LogOut } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="relative group">
        {hasMultipleImages ? (
          <div className="flex overflow-x-auto snap-x snap-mandatory gap-2 px-4 pt-4 pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {images.map((img: string, idx: number) => (
              <img
                key={idx}
                src={img}
                alt={`${item.title} - ${idx + 1}`}
                className="w-[85%] h-56 object-cover rounded-2xl flex-shrink-0 snap-center border border-gray-100"
        ...
        <div className={`absolute ${hasMultipleImages ? 'top-6 left-6' : 'top-4 left-4'} px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full text-xs font-bold text-blue-700 shadow-sm`}>
          {new Date(item.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
        </div>
      </div>
      <div className={`p-6 flex flex-col flex-1 ${hasMultipleImages ? 'pt-2' : ''}`}>
...
const CommentSection = () => {
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      const newComment = {
        userId: user.uid,
        name: user.displayName || 'Warga RW 015',
        text: text.trim(),
    ...
    <section className="py-16 bg-white border-t border-gray-100">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Komentar & Diskusi</h2>
        </div>
          ...
          <>
            <div className="flex items-center justify-between mb-6 bg-gray-50 p-4 rounded-xl border border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-gray-900">{user.displayName || 'Warga RW 015'}</p>
                  <p className="text-xs text-gray-500">{user.email}</p>
                </div>
              </div>
              <button
                onClick={() => signOut(auth)}
                className="px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors flex items-center gap-2 font-medium"
              >
                <LogOut size={16} />
                Keluar
              </button>
            ...
            <form onSubmit={handleAddComment} className="mb-10">
              <div className="relative">
                <textarea
                  ...
                  className="w-full px-4 py-4 pr-12 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all resize-none min-h-[120px]"
                  required
                />
                <button
                  type="submit"
                  disabled={!text.trim()}
                  className="absolute bottom-4 right-4 p-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Send size={18} />
                </button>
              </div>
            ...
            <div className="space-y-6">
              {comments.map((comment) => (
                <div key={comment.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-gray-500">{new Date(comment.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                      </div>
                            ...
                            <div key={reply.id} className="flex items-start gap-3">
                              ...
                              <div className="flex-1 bg-gray-50 rounded-2xl rounded-tl-none p-3">
                                <div className="flex items-center justify-between mb-1">
                                  ...
                                  <span className="text-[10px] text-gray-500">{new Date(reply.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</span>
                                </div>
                      ...
                      {replyingTo === comment.id && (
                        <form onSubmit={(e) => handleAddReply(comment.id, e)} className="mt-4 flex gap-2">
                          <input
                            type="text"
                            value={replyText}
                            onChange={(e) => setReplyText(e.target.value)}
                            placeholder="Tulis balasan..."
                            className="flex-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all text-sm"
                            autoFocus
                            required
                          />
                          <button
              ...
              {comments.length === 0 && (
                <div className="text-center py-12 bg-gray-50 rounded-2xl border border-gray-100">
                  ...
                  <p className="text-gray-500">Belum ada komentar. Jadilah yang pertama berdiskusi!</p>
                </div>
              )}
            </div>
          </>
